...
| --- | --- | --- |
| exe | **0.28 MB** | 2.07 MB |
| Private (起動直後) | **1.4 MB** | 4.3 MB |
| Working Set (起動直後) | **7.7 MB** | 10.9 MB |
| ビルドに要るもの | Rust + MSVC リンカ | .NET 9 SDK + VS C++ ビルドツール |

挙動と設定ファイルは同一で、**同じ E2E テストの両方を通しています**。
Rust 版が本命で、C# 版は移植元として残してあります。

Working Set は**トレイメニューやダイアログを一度開くと 20MB 台まで増えます**
(shell32/comctl32 などが写像されるため)。増える分は共有 DLL のページで、
メモリ圧迫時には OS が回収します。SandS 固有の消費は Private の方で、
実使用後でも 3〜4MB 程度です。上表は起動直後・無操作の値なので、
常駐中の Working Set をそのまま表すものではありません。
...
copy sands-rs\target\release\sands.exe "$env:LOCALAPPDATA\SandS\"
